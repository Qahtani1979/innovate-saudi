...
function CampaignPlanner({ embedded = false }) {
  ...
  const { data: programs = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return all.filter(p => p.program_type === 'campaign' || p.program_type === 'challenge');
    }
  });
...
Strategic Themes: ${strategicPlans[0]?.strategic_themes?.map(t => t.name_en).join(', ') || 'Smart Cities, Sustainability, Quality of Life'}
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-pink-50 to-white">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-3xl font-bold text-pink-600">{programs.length}</p>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-white">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-3xl font-bold text-purple-600">
              {programs.reduce((sum, p) => sum + (p.events?.length || 0), 0)}
            </p>
            ...
            <div className="flex items-center justify-between pt-4 border-t">
              ...
              {currentStep < 4 ? (
                ...
                <Button
                  ...
                  disabled={!campaignData.name_en || createCampaignMutation.isPending}
