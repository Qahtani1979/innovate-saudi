...
function AdminPortal() {
  ...
  const { stats, recentActivities, isLoading } = useAdminData();
  ...
  const {
    ...
    experts = 0,
    rdProjects = 0,
    pendingApprovals = { challenges: 0, pilots: 0, programs: 0, total: 0 }
  } = stats || {};

  const citizenIdeasCount = citizenIdeas || 0;
  ...
  const adminSections = [
    {
      ...
      color: 'blue',
      items: [
        { name: t({ en: 'Challenges', ar: 'التحديات' }), count: challenges, page: 'Challenges', icon: AlertCircle, color: 'red' },
        { name: t({ en: 'Solutions', ar: 'الحلول' }), count: solutions, page: 'Solutions', icon: Lightbulb, color: 'green' },
        { name: t({ en: 'Pilots', ar: 'التجارب' }), count: pilots, page: 'Pilots', icon: TestTube, color: 'blue' },
        { name: t({ en: 'Organizations', ar: 'الجهات' }), count: organizations, page: 'Organizations', icon: Users, color: 'purple' },
        { name: t({ en: 'Programs', ar: 'البرامج' }), count: programs.length, page: 'Programs', icon: Calendar, color: 'amber' },
        { name: t({ en: 'R&D Projects', ar: 'مشاريع البحث' }), count: rdProjects, page: 'RDProjects', icon: TestTube, color: 'indigo' }
      ]
    },
    {
      ...
      color: 'green',
      items: [
        { name: t({ en: 'Municipalities', ar: 'البلديات' }), count: municipalities, page: 'DataManagementHub', icon: MapPin, color: 'teal' },
    ...
    },
    {
      ...
      items: [
        ...
        { name: t({ en: 'Experts', ar: 'الخبراء' }), count: experts, page: 'ExpertRegistry', icon: Award, color: 'amber' },
    ...
    },
    {
      ...
      color: 'yellow',
      items: [
        { name: t({ en: 'Ideas', ar: 'الأفكار' }), count: citizenIdeasCount, page: 'IdeasManagement', icon: Lightbulb, color: 'yellow' },
    ...
    },
    {
      ...
      color: 'slate',
      items: [
        { name: t({ en: 'Approvals', ar: 'الموافقات' }), count: pendingApprovals.total, page: 'ApprovalCenter', icon: CheckCircle2, color: 'green' },
  ...
  return (
    <PageLayout>
      <PageHeader
        ...
        subtitle={{ en: 'Platform administration and system configuration', ar: 'إدارة المنصةوتكوين النظام' }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-bold text-amber-900">
                  {pendingApprovals.total} {t({ en: 'Pending Approval(s)', ar: 'موافقات معلقة' })}
                </p>
                <p className="text-sm text-amber-700">
                  {pendingApprovals.challenges} challenges • {pendingApprovals.pilots} pilots • {pendingApprovals.programs} programs
                </p>
              </div>
        ...
        <Card className="bg-gradient-to-br from-red-50 to-white border-red-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-red-600">{challenges}</p>
              <p className="text-sm text-slate-600">{t({ en: 'Challenges', ar: 'التحديات' })}</p>
              <div className="mt-2 text-xs text-slate-500">
                {stats?.pendingChallenges || 0} {t({ en: 'pending', ar: 'معلقة' })}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-white border-blue-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <div className="mt-2 text-xs text-slate-500">
                {stats?.activePilots || 0} {t({ en: 'active', ar: 'نشطة' })}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-green-50 to-white border-green-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-green-600">{solutions}</p>
              <p className="text-sm text-slate-600">{t({ en: 'Solutions', ar: 'الحلول' })}</p>
              <div className="mt-2 text-xs text-slate-500">
                {stats?.verifiedSolutions || 0} {t({ en: 'verified', ar: 'معتمدة' })}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-white border-purple-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-purple-600">{organizations}</p>
              <p className="text-sm text-slate-600">{t({ en: 'Organizations', ar: 'الجهات' })}</p>
              <div className="mt-2 text-xs text-slate-500">
                {stats?.partners || 0} {t({ en: 'partners', ar: 'شركاء' })}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-amber-50 to-white border-amber-200">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-amber-600">{municipalities}</p>
              <p className="text-sm text-slate-600">{t({ en: 'Municipalities', ar: 'البلديات' })}</p>
              <div className="mt-2 text-xs text-slate-500">
                {stats?.activeMunicipalities || 0} {t({ en: 'active', ar: 'نشطة' })}
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            {recentActivities.slice(0, 8).map((activity) => (
              <div key={activity.id} className="p-3 bg-slate-50 rounded-lg border flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-slate-900 font-medium">{activity.action?.replace(/_/g, ' ')}</p>
                  <p className="text-xs text-slate-600">{activity.entity_type} • {activity.user_email || 'System'}</p>
                  <p className="text-xs text-slate-400">{new Date(activity.created_at || activity.created_date).toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex items-center justify-between p-2 bg-slate-50 rounded">
              ...
              <Badge>{Math.max(0, challenges - (stats?.pendingChallenges || 0))}→{stats?.pendingChallenges || 0}</Badge>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
              ...
              <span className="text-2xl font-bold text-purple-600">{citizenIdeasCount}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
              <span className="text-sm text-slate-700">{t({ en: 'Approval Rate', ar: 'معدل القبول' })}</span>
              <span className="text-2xl font-bold text-yellow-600">
                {citizenIdeasCount > 0 ? Math.round((stats.approvedIdeas / citizenIdeasCount) * 100) : 0}%
              </span>
            </div>
          </CardContent>
        </Card>
