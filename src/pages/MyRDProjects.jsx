...
import { useMyRDProjects } from '@/hooks/useMyRDProjects';
...
import {
  ...
  FileText, DollarSign, Clock, Lightbulb, ArrowRight, Loader2
} from 'lucide-react';
...
function MyRDProjects() {
  ...
  const { invokeAI, status, rateLimitInfo } = useAIWithFallback();
  ...
  const {
    ...
    totalPages,
    generateInsights
  } = useMyRDProjects(user, invokeAI);

  // Client-side filtering for the CURRENT PAGE of projects
  ...
  return (
    <PageLayout>
      ...
      {/* Summary Cards (Powered by Stats Query) */}
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card className="border-2 border-indigo-200 bg-gradient-to-br from-indigo-50 to-white">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-3xl font-bold text-indigo-600">{stats.activeCount}</p>
          ...
          <Card className="border-2 border-orange-200 bg-gradient-to-br from-orange-50 to-white">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-3xl font-bold text-orange-600">{stats.milestonesNextWeekCount}</p>
          ...
          <Card className="border-2 border-green-200 bg-gradient-to-br from-green-50 to-white">
            <CardContent className="pt-6 text-center">
              ...
              <p className="text-3xl font-bold text-green-600">{stats.publicationsTotal}</p>
      ...
      {!isLoading && stats.criticalMilestones?.length > 0 && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-bold text-red-900 mb-2">
                  {t({ en: `${stats.criticalMilestones.length} Critical Milestones This Week`, ar: `${stats.criticalMilestones.length} معلم حرج هذا الأسبوع` })}
                </h3>
                <div className="space-y-2">
                  {stats.criticalMilestones.map((m, i) => (
      ...
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="border p-4 rounded-lg bg-white space-y-3">
