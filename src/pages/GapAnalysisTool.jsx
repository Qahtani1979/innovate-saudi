...
function GapAnalysisTool() {
  ...
  const runGapAnalysis = async () => {
    ...
    const result = await invokeAI({
      prompt: GAP_ANALYSIS_PROMPT_TEMPLATE({
        sectorData,
        challenges: {
          total: challenges.length,
          approved: challenges.filter(c => c.status === 'approved').length,
          highPriority: challenges.filter(c => c.priority === 'tier_1').length
        },
        pilots: {
          total: pilots.length,
          active: pilots.filter(p => p.stage === 'active' || p.stage === 'monitoring').length,
          atRisk: pilots.filter(p => p.risk_level === 'high').length
        },
        solutions: {
          total: solutions.length,
          marketReady: solutions.filter(s => s.maturity_level === 'market_ready' || s.maturity_level === 'proven').length
        },
        rdProjects: {
          total: rdProjects.length
        }
      }),
      response_json_schema: GAP_ANALYSIS_SCHEMA
    });
