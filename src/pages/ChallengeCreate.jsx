...
import { Sparkles, ArrowRight, ArrowLeft, Save, Loader2, Target, Plus, Trash2, Globe, AlertCircle, Languages } from 'lucide-react';
...
function ChallengeCreatePage() {
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      const prompt = `You are a municipal innovation expert analyzing a challenge for Saudi municipalities.

INPUT:
Municipality: ${municipality?.name_en} (${municipality?.name_ar || ''})
User's Description: ${initialThoughts || formData.description_en || formData.title_en}
${ideaContext}
...
3. Descriptions (EN + AR, 250+ words each): Comprehensive problem description focusing on:
   - What the problem is (factual, data-driven)
...
6. Desired Outcomes (EN + AR): OPEN-ENDED end state goal (NOT specific solution approach)
...
10. Subsector ID: Best match from available subsectors for that sector
...
24. Priority: tier_1/tier_2/tier_3/tier_4 (based on severity+impact)
