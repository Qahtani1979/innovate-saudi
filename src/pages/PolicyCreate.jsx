...
function PolicyCreate() {
  ...
  const { useChallenges, usePilots, useRDProjects } = useMatchingEntities();
  ...
  /* Refactored to replace inline useMutation */
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Translate
      const translations = await translatePolicy.mutateAsync({
      ...
      const policy = await createPolicy.mutateAsync({
        ...
        description_ar: formData.description_ar,
        description_en: translations.description_en || translations.recommendation_text_en,
        priority: formData.priority,
        status: formData.status,
        recommendation_type: formData.policy_type,
        source_entity_id: linkedEntities.length > 0 ? linkedEntities[0].id : null,
        source_entity_type: linkedEntities.length > 0 ? linkedEntities[0].type : null,
        supporting_evidence: {
          regulatory_framework: formData.regulatory_framework,
          regulatory_change_needed: formData.regulatory_change_needed,
          timeline_months: formData.timeline_months,
          implementation_complexity: formData.implementation_complexity,
  ...
  const handleAIAssist = async () => {
    const inputText = currentStep === 1 ? initialThoughts : (formData.title_ar || formData.description_ar);
    ...
    } = await import('@/lib/ai/prompts/policies/generator');

    let contextPrompt = POLICY_GENERATOR_PROMPT_TEMPLATE({
      initialThoughts,
      formData,
      linkedEntities
    });

    // Add context for all linked entities using centralized templates
    linkedEntities.forEach((link, idx) => {
      ...
      if (link.type === 'challenge') {
        ...
        if (challenge) {
          contextPrompt += POLICY_LINKED_ENTITY_TEMPLATES.challenge(challenge, idx);
        }
      }
      ...
      if (link.type === 'pilot') {
        ...
        if (pilot) {
          contextPrompt += POLICY_LINKED_ENTITY_TEMPLATES.pilot(pilot, idx);
        }
      }
      ...
      if (link.type === 'rd_project') {
        ...
        if (rd) {
          contextPrompt += POLICY_LINKED_ENTITY_TEMPLATES.rd_project(rd, idx);
        }
      }
      ...
      if (link.type === 'program') {
        ...
        if (prog) {
          contextPrompt += POLICY_LINKED_ENTITY_TEMPLATES.program(prog, idx);
        }
      }
    });

    // Import the generation instructions
    const { POLICY_GENERATION_INSTRUCTIONS } = await import('@/lib/ai/prompts/policies/generator');

    const { success, data } = await invokeAI({
      prompt: `${contextPrompt}${POLICY_GENERATION_INSTRUCTIONS}`,
      ...
      system_prompt: 'You are an expert policy analyst specializing in Saudi municipal regulations. Create structured, actionable policy frameworks.'
    });
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const uploadPromises = files.map(file => uploadAttachment.mutateAsync(file));
  ...
  return (
    <PageLayout className="max-w-5xl mx-auto">
      <PageHeader
        ...
        action={<Link to={createPageUrl('PolicyHub')}>
          <Button variant="ghost" size="sm">
            <ArrowLeft className="h-4 w-4 mr-2" />
            {t({ en: 'Back', ar: 'رجوع' })}
          </Button>
        </Link>} subtitle={undefined} actions={undefined} children={undefined} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({ ...formData, timeline_months: parseInt(e.target.value) || 0 })}
                  placeholder="6"
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="flex justify-between gap-3 pt-6 border-t" dir={isRTL ? 'rtl' : 'ltr'}>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={isSubmitting || !formData.title_ar}
