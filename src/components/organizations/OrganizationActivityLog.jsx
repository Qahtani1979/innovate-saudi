...
import { useOrganizationActivities } from '@/hooks/useOrganizations';

export default function OrganizationActivityLog({ organizationId }) {
  const { t, language } = useLanguage();


  const { data: activities = [] } = useOrganizationActivities(organizationId);
              ...
              <div key={idx} className="flex gap-3 p-3 bg-slate-50 rounded-lg border">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-medium">{activity.user_email || 'Unknown User'}</span>
